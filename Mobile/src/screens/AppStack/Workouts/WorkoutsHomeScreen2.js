...
class WorkoutsHomeScreen2 extends React.PureComponent {
  ...
  fetchCategories = async () => {
    ...
    this.unsubscribeCategories = await db
  ...
  handleClick(mainIndex, subIndex) {
    ...
    this.props.navigation.navigate("WorkoutsSelection", {
      ...
      selectedSubCategory:
        this.state.workoutCategories[mainIndex].subCategories[subIndex],
    });
  }

  render() {
    const { loading, helperModalVisible, toggleList, workoutCategories } =
      this.state;
    ...
    return (
      <ScrollView
        ...
        onContentSizeChange={() => {
          if (
            this.state.toggleList[1] ||
            this.state.toggleList[2] ||
            this.state.toggleList[3]
          )
                    ...
                    {toggleList[index] && (
                      <View style={{ marginBottom: 20, paddingLeft: 5 }}>
                        {work.subCategories.map((data, i) => (
                          ...
                          >
                            <Text style={WorkoutScreenStyle.subTitleText}>
                              {data.displayName}
                            </Text>
                          </TouchableOpacity>
                        ))}
