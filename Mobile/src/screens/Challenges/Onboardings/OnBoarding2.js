...
export default class OnBoarding2 extends Component {
  ...
  onFocusFunction = () => {
    ...
    if (data["challengeData"]["onBoardingInfo"]) {
      const storedPreferences =
        data["challengeData"]["onBoardingInfo"]["dietryPreferences"];
  ...
  goToScreen(type) {
    ...
    const selectedPreferences = dietryPreferences.filter(
      (item) => item.selected
    );
  ...
  render() {
    let { btnDisabled, dietryPreferences, challengeData } = this.state;
              ...
              {dietryPreferences.map((item, i) => {
                return (
                  <TouchableOpacity
                    ...
                    activeOpacity={0.8}
                    style={[
                      globalStyle.selectBox,
                      item.selected ? globalStyle.selectedBox : {},
                    ]}
