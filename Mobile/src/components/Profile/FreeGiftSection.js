...
function FreeGiftSection({ isUnlocked, giftName, minimumInvites, promoCode }) {
  return (
    <View
      style={isUnlocked ? styles.containerUnlocked : styles.containerLocked}
    >
      {isUnlocked ? (
        <TouchableOpacity
          onPress={() => {
        ...
        <Icon name="padlock" size={20} color={colors.white} />
      )}
      <Text style={styles.giftName}>
        {isUnlocked
          ? giftName
          : `Invite ${minimumInvites} friends to unlock this gift`}
      </Text>
    </View>
  );
}
