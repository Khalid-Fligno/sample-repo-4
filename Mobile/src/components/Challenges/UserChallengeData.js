...
export default createUserChallengeData = (
  data,
  challengeDate,
  stringDate3,
  TODAY1
  ...
  const currentChallengeDay = getCurrentChallengeDay(stringDate3, TODAY1);
  ...
  const phases = data.phases.map((res) => {
    return {
      ...
      startDate: moment(challengeDate, "YYYY-MM-DD")
        .add(res.startDay - 1, "days")
        .format("YYYY-MM-DD"),
      endDate: moment(challengeDate, "YYYY-MM-DD")
        .add(res.endDay - 1, "days")
        .format("YYYY-MM-DD"),
