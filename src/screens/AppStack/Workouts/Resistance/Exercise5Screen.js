...
export default class Exercise5Screen extends React.PureComponent {
  ...
  manageVideoCache = async () => {
    ...
    if (setCount === 0) {
      FileSystem.deleteAsync(`${FileSystem.documentDirectory}exercise-4.mp4`, { idempotent: true });
    }
    if (setCount === 2) {
      FileSystem.downloadAsync(
        exerciseList[5].videoURL,
        `${FileSystem.documentDirectory}exercise-6.mp4`,
      );
    }
  }
  startTimer = () => {
        ...
        >
          <View>
            <Video
              ...
              source={{ uri: `${FileSystem.documentDirectory}exercise-5.mp4` || exerciseList[4].videoURL }}
              rate={1.0}
              volume={1.0}
              isMuted={false}
              resizeMode="contain"
          ...
          <PauseButtonRow
            ...
            nextExerciseName={exerciseList[5].name}
          />
          <WorkoutPauseModal
